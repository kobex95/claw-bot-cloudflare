...
<head>
  ...
  <title>claw 智能助手 - Cloudflare Workers AI</title>
  <style>
...
<body>
  ...
  <div class="chat-container">
    <div class="messages" id="messages">
      <div class="welcome">
        <h2>👋 欢迎使用 Claw 智能助手</h2>
        <p>我是你的 AI 助手，基于 Cloudflare Workers 构建。</p>
        <p>输入消息开始对话吧！</p>
        <div class="hint">
          💡 我可以帮你：<br>
          • 回答问题和提供信息<br>
    ...
    <div class="input-area">
      <input type="text" id="messageInput" placeholder="输入消息，按 Enter 发送..." autocomplete="off">
  ...
  <script>
    ...
    // 会话管理
    let sessionId = localStorage.getItem('claw_session_id') || generateSessionId();
    ...
    async function sendMessage() {
      ...
      try {
        ...
        addMessage(data.reply || data.message || '无响应', false);
...
async function handleChatMessage(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  try {
    ...
    if (!session.id) session.id = effectiveSessionId;
    if (!session.userId) session.userId = 'web-user';
    if (!session.chatId) session.chatId = effectiveSessionId;
    ...
    await saveSessionWithFallback(effectiveSessionId, session, env);
